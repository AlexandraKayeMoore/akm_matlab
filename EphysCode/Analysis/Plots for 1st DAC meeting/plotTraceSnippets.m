function plotTraceSnippets(filepath,filename,snippets,mvLineAt)
...
t_s=[1:length(in.data.voltage)]/in.settings.sampRate;

if 0
    data=in.data.voltage;
    low_pass_cutoff=1e3; % 3 kHz
    fprintf('\nLow-pass filtering at %d Hz\n',low_pass_cutoff);
    [b,a]=butter(1,low_pass_cutoff/(in.settings.sampRate/2),'low');
    filteredTrace_LP=filtfilt(b,a,data);  
    in.data.voltage=filteredTrace_LP;
end
...
%% Voltage trace & line - rescale and plot
    ...
    vSnip=in.data.voltage(startSample:endSample);
    ...
    samples_per_frame=floor(in.settings.sampRate/in.settings.camRate);
