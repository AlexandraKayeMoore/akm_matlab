function plotEventResponses_FFflash(filepath,daqfilename,eventsfilename,baseline_mV,plus_minus_mV)
%% plotEventResponses_FFflash(filepath,daqfilename,eventsfilename,baseline_mV,plus_minus_mV)
% 10/23/18
% Example arguments --
...
load(eventsfilename);
load(daqfilename);
fInfoFilename=strsplit(stimulus.panelParams.path2fInfo,'\');
fInfoFilename=fInfoFilename{end};
...
for thisEvent=1:length(eventNames) % For each unique event...
    ...
    if thisEvent==1;
        set(gca,'position',[0.088 0.222 0.821 0.545]);
        xlabel('time (s)');
    end
