%% Elevation scan with 4x4 object - version 2
% Position functions for use with patterns 027 (bright object) and 028
% (dark object). 
% 
% speed = 36 dps
% isi = 3 seconds
% 3 reps per direction, per elevation 
% scan from x=2-->70 at y=2,6,10,14
...
fDescription='elevation_scan_4x4_36dps';
xfuncNum='039';
yfuncNum='040';
...
x_locations=[2:70]; % scan in azimuth
y_locations=[2,6,10,14]; % elevations

events=struct();
i=1;

% object scans left --> right @ each elevation
for yp=1:length(y_locations)
    for r=1:reps_per_stim
        events(i).name='scan_LR';
...
% object scans right --> left @ each elevation
for yp=1:length(y_locations)
    for r=1:reps_per_stim
        events(i).name='scan_RL';
